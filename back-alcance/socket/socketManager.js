...
const socketManager = (httpServer, whitelist) => {
...
io.on("connection", (socket) => {
    ...
    socket.on("join_room", (data) => {
      ...
      // console.log("User " + data.author + " with Id " + socket.id + "have joined room: " + data.room);
    ...
    socket.on("disconnect", () => {           
      ...
      // console.log(users);
      if (found && found.room) socket.to(found.room).emit("have_left_room", {author: found.user, message: "left room"});
      if (found && found.room) io.in(found.room).emit("update_users_list", users);
    })
